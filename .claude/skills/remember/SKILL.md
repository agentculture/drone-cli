...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope drone-cli
...
scope:** the default personal scope is **private** (`--scope drone-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `drone-cli`). Private records are served only to a recall in the same
