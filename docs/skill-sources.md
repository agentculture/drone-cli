...
drone-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. drone-cli
...
#   - consumer-identifying prose: `guildmaster` → `drone-cli` (NOT
...
If a re-sync would lose a drone-cli adaptation, lift the change
...
#     → `which drone-cli vendors from guildmaster`
